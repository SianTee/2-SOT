...
class OntReasoner():
    def __init__(self):
        onto_path.append('data/ontology')  # Path to ontology
        ...
        self.sentence_vector , self.target_vector, self.polarity_vector, \
        self.polarity, self.position = [],[],[],[],[]
    ...
    def predict_sentiment(self, sentence, target, onto, use_ml, position, types1, types2, types3):
        words_in_sentence = sentence.split()
        self.sencount += 1

        lemma_of_words_with_classes, words_with_classes, words_classes,\
        target_class = self.get_class_of_words(words_in_sentence, target)
        ...
        for x in range(len(words_with_classes)):
            ...
            if lemma_of_word in types1:
                found_positive, found_negative = self.get_sentiment_of_class(
                        positive_class, negative_class, word_class,
                        negated, False)
            ...
            if lemma_of_word in types2:
                if self.category_matches(target_class, word_class):
                    ...
                    self.get_sentiment_of_class(positive_class, negative_class,
            ...
            if lemma_of_word in types3:
                ...
                found_positive, found_negative = self.get_sentiment_of_class(
                        positive_class, negative_class, new_class, negated,
                        True)
                found_positive_list.append(found_positive)
                found_negative_list.append(found_negative)
        ...
        # Create remaining vector for Neural Method
            ...
            self.sencount += -1
            self.polarity_vector = np.delete(self.polarity_vector,
                                             self.sencount, 0)
            self.remaining_sentence_vector.append(sentence)
            self.remaining_target_vector.append(target)
            self.remaining_pos_vector.append(position)
        else:
            self.prediction_vector.append(self.get_majority_class(
                    self.polarity_vector))
        ...
        for word in words_in_sentence:
            ...
            for value in list(self.my_dict.values()):
                if lemma_of_word in value:
                    lemma_of_word_class = list(self.my_dict.keys())[
                            list(self.my_dict.values()).index(value)]
    ...
    def is_negated(self, word, words_in_sentence):
        ...
        path_to_jar = os.path.join('data', 'ontology',
                                   'stanford-parser-full-2018-02-27',
                                   'stanford-parser.jar')
        path_to_models_jar = os.path.join('data', 'ontology',
                                          'stanford-parser-full-2018-02-27',
        ...
        if negated == False and any(x in s for x in negations for s in
                                    words_in_sentence):
    ...
    def get_sentiment_of_class(self, positive_class, negative_class,
                               onto_class, negated, type3):
    ...
    def category_matches(self, target_class, onto_class):
        ...
        common_list = list(set(target_mentions).intersection(
                set(onto_mentions)))
    ...
    def create_types(self):
        ...
        for c in classes:
            ...
            remove_words = ['Property', "Mention", "Positive", "Neutral",
                            "Negative"]
            ...
            for name_ancestor in ancestors_list:
                if boolean == 1:
                    if "Generic" in name_ancestor:
                        ...
                        boolean = 0
                    elif "Positive" in name_ancestor or "Negative" in\
                    name_ancestor:
    ...
    def run(self, path, use_ml = True, cross_val = False, j = 0):
        types1, types2, types3 = self.create_types()

        punctuation_and_numbers = '[–()?:;,.!/"\'*$0123456789]'

        input_df = pd.read_csv(path, skiprows=0, encoding='utf-8')

        # polarity
        polarity_one_hot_df = pd.get_dummies(input_df['Polarity'].astype(
                pd.api.types.CategoricalDtype(
                        categories = ['positive','neutral','negative'])))
        self.polarity_vector = polarity_one_hot_df.to_numpy(dtype=int)

        # load target and sentence and remove punctuation and numbers
        self.target_vector = input_df['Aspect text']\
        .str.lower().str.replace(punctuation_and_numbers,'').to_numpy()
        self.sentence_vector = input_df['Sentence text']\
        .str.lower().str.replace(punctuation_and_numbers,'').to_numpy()

        self.position = input_df['Position'].to_numpy(dtype=int)

        printProgressBar(0, len(self.sentence_vector), suffix = 'completed',
                         length = 50)

        for x in range(len(self.sentence_vector)):  # For each sentence
            self.predict_sentiment(self.sentence_vector[x],
                                   self.target_vector[x],
                                   self.onto, use_ml,
                                   self.position[x], types1, types2, types3)
            printProgressBar(x+1, len(self.sentence_vector),
        ...
        correct = bool_vector.astype(float)

        accuracy = sum(correct)/(self.sentence_vector.size - len(
                self.remaining_sentence_vector))

        timeEnd = time.time()

        print("Ontology accuracy: ", accuracy)
        print('RunTime: ', (timeEnd - self.timeStart))
        print('Chosen by majority:', len(self.majority_count))

        # Convert to numpy array to save the output
        self.remaining_sentence_vector = np.array(
                self.remaining_sentence_vector)
        self.remaining_target_vector = np.array(self.remaining_target_vector)
        self.remaining_polarity_vector = np.array(
                self.remaining_polarity_vector)
        self.remaining_pos_vector = np.array(self.remaining_pos_vector)

        # Save the results and remaining examples to file
        if use_ml == True:
            remaining_path = os.path.join('data', 'ontology',
                                          os.path.splitext(
                                                    os.path.basename(path))[0])
            with open(remaining_path, 'wb') as f:
                dill.dump([self.remaining_pos_vector, accuracy, sum(correct),
                           self.sentence_vector.size,
                           len(self.remaining_pos_vector)], f)

        if cross_val:
            if use_ml == True:
                print(self.remaining_pos_vector)
                outF= open("data/programGeneratedData/crossValidation"+str(
                        FLAGS.year)+'/cross_val_remainder_'+str(j)+'.txt', "w")
        ...
        return accuracy, sum(correct), self.sentence_vector.size, len(self.remaining_pos_vector)